...
class LEDFxConfig extends ChangeNotifier {
  ...
  Future<void> ensureDummyVisualizerDevice() async {
    devices.removeWhere((d) => d["id"] == "dummyViz");

    final dummyVizVirtual = virtuals.firstWhere((v) => v["id"] == "dummyVizVirtual", orElse: () => {});
    Map<String, dynamic>? dummyEffect;
    if (dummyVizVirtual.isNotEmpty) {
      dummyEffect = dummyVizVirtual["activeEffect"];
    }

    virtuals.removeWhere((v) => v["id"] == "dummyVizVirtual");

    devices.insert(0, {
      "id": "dummyViz",
      "config": DeviceConfig(type: 'dummy', pixelCount: 300, rgbwLED: false, name: 'Dummy Visualizer').toJson(),
    });

    virtuals.insert(0, {
      "id": "dummyVizVirtual",
      "active": true,
      "config": VirtualConfig(name: 'Dummy Visualizer Virtual', deviceID: "dummyViz", autoGenerated: true).toJson(),
      "segments": [SegmentConfig("dummyViz", 0, 299, false).toJson()],
      "activeEffect":
          dummyEffect ?? EffectConfig(name: 'Dummy Viz Effect', type: EffectType.wavelength, mirror: true).toJson(),
    });
  }
}
...
class LEDFx {
  ...
  Future<void> start([bool pauseAll = false]) async {
    ...
    await config.ensureDummyVisualizerDevice();
