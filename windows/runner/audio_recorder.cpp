...
std::vector<AudioDevice> AudioRecorder::EnumerateDevices(EDataFlow dataFlow) {
  ...
  HRESULT hr = device_enumerator_->EnumAudioEndpoints(
      dataFlow, DEVICE_STATE_ACTIVE, &device_collection);
  ...
  if (SUCCEEDED(hr)) {
    ...
    device_enumerator_->GetDefaultAudioEndpoint(dataFlow, eConsole,
                                                &default_device);
    ...
    for (UINT i = 0; i < device_count; i++) {
      ...
      if (SUCCEEDED(hr)) {
        ...
        if (format_blob.size() >= sizeof(WAVEFORMATEX)) {
          WAVEFORMATEX *wfx =
              reinterpret_cast<WAVEFORMATEX *>(format_blob.data());
...
std::string AudioRecorder::GetDeviceProperty(IMMDevice *device,
...
void AudioRecorder::Start(const std::string &deviceId,
                          const std::string &captureType, int sampleRate,
...
void AudioRecorder::AudioCaptureThread() {
  if (!device_enumerator_) {
    if (error_callback_)
      error_callback_("Device enumerator not available");
    return;
  }
  ...
  if (SUCCEEDED(hr)) {
    try {
      ...
      if (error_callback_)
        error_callback_("Capture error: " + std::string(e.what()));
    }
    device->Release();
  } else {
    if (error_callback_)
      error_callback_("Failed to get audio device");
  }
}
...
void AudioRecorder::CaptureAudio(IMMDevice *device, bool loopback) {
  ...
  if (FAILED(hr)) {
    if (error_callback_)
      error_callback_("Failed to activate audio client");
    return;
  }
  ...
  custom_format.nAvgBytesPerSec =
      custom_format.nSamplesPerSec * custom_format.nBlockAlign;
  custom_format.cbSize = 0;

  WAVEFORMATEX *mix_format = &custom_format;
  WAVEFORMATEX *closest_supported = nullptr;
  hr = audio_client_->IsFormatSupported(AUDCLNT_SHAREMODE_SHARED, mix_format,
                                        &closest_supported);
  if (hr == S_FALSE && closest_supported) {
    mix_format = closest_supported;
  } else if (FAILED(hr)) {
    if (error_callback_)
      error_callback_("Requested audio format not supported");
    return;
  }

  HANDLE hEvent = CreateEvent(nullptr, FALSE, FALSE, nullptr);
  if (!hEvent) {
    if (closest_supported)
      CoTaskMemFree(closest_supported);
    if (error_callback_)
      error_callback_("Failed to create event handle");
    return;
  }
  ...
  REFERENCE_TIME buffer_duration =
      CalculateBufferDuration(device_sample_rate, target_blocksize_);

  hr = audio_client_->Initialize(AUDCLNT_SHAREMODE_SHARED, streamFlags,
  ...
  if (FAILED(hr)) {
    ...
    if (closest_supported)
      CoTaskMemFree(closest_supported);
    if (error_callback_)
      error_callback_("Failed to initialize audio client");
    return;
  }
      ...
      while (packet_length != 0 && is_capturing_) {
        ...
        if (SUCCEEDED(hr)) {
          ...
          size_t frames_needed =
              (target_blocksize_ > 0) ? target_blocksize_ : frames_available;
...
REFERENCE_TIME AudioRecorder::CalculateBufferDuration(int device_sample_rate,
                                                      int target_blocksize) {
  if (target_blocksize <= 0)
    return 10000000; // 1s
  double duration_seconds = static_cast<double>(target_blocksize) /
                            static_cast<double>(device_sample_rate);
  REFERENCE_TIME buffer_duration =
      static_cast<REFERENCE_TIME>(duration_seconds * 10000000.0);
